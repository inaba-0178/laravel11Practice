...
return new class extends Migration
{
    ...
    {
        Schema::connection('mst')->create('opr_cars', function (Blueprint $table) {
            $table->id();
            $table->foreignId('dealer_id')
                ->constrained('opr_car_dealers')
                ->cascadeOnDelete()
                ->cascadeOnUpdate()
                ->comment('販売店ID');
            ...
            $table->bigInteger('region_id')->comment('都道府県ID');
    ...
    {
        Schema::connection('mst')->dropIfExists('opr_cars');
    }
};