<x-filament-panels::page>
    @php
        ...
        $currentFlaggedImages  = $flaggedImages[$currentCarId] ?? [];
        $currentRejectionItems = $rejectionItems[$currentCarId] ?? [];
        $currentGeneralComment = $generalComments[$currentCarId] ?? '';
    @endphp
    ...
    <div style="background: white; border-radius: 12px; border: 0.5px solid #e5e7eb; padding: 14px 18px; margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center;">
        <div>
            <p style="font-size: 14px; font-weight: 600; color: #111827; margin: 0;">
                {{ $record->dealer?->name }} ／ バッチ #{{ $record->id }}
            </p>
            <p style="font-size: 11px; color: #6b7280; margin: 4px 0 0;">
                アップロード日時：{{ $record->uploaded_at?->format('Y/m/d H:i') }}　／　
                合計：{{ $record->total_count }}台　承認待ち：{{ $record->pending_count }}台　
                承認済み：{{ $record->approved_count }}台　差し戻し：{{ $record->rejected_count }}台
            </p>
            @php
                $canPublish = $record->cars->every(fn($c) => $c->status === \App\Constants\CarStatus::APPROVED_PENDING);
            @endphp
        </div>
    </div>
    ...
    <div style="border: 2px solid #e5e7eb; border-radius: 12px; overflow: hidden;">
        ...
        {{-- 車両タブ --}}
        <div style="display: flex; gap: 6px; flex-wrap: wrap; padding: 12px 12px 0; border-bottom: 2px solid #e5e7eb; background: transparent;">
            ...
            >
                ...
                <div>
                    <p style="font-size: 12px; font-weight: {{ $isActive ? '600' : '400' }}; color: {{ $isActive ? '#be185d' : '#374151' }}; margin: 0 0 2px;">
                        ...
                        @if($hasContent && $tabCar->status === 'pending')
        ...
        <div style="padding: 20px;">
            <div style="display: grid; grid-template-columns: 1fr 300px 280px; gap: 14px; align-items: start;">
                ...
                {{-- ==================== 左：車両詳細 ==================== --}}
                <div wire:key="car-detail-{{ $currentCarId }}" style="display: flex; flex-direction: column; gap: 14px;">
                    ...
                    @if(count($images) > 0)
                    <div
                        wire:key="car-images-{{ $currentCarId }}"
                        ...
                        x-data="{
                            ...
                            perPage: 10,
                            images: @js($images),
                            flagged: @js($currentFlaggedImages),
                ...
                <div style="position: sticky; top: 20px; display: flex; flex-direction: column; gap: 10px;">
                    <div style="background: white; border-radius: 12px; border: 0.5px solid #e5e7eb; overflow: hidden;">
                        <div style="padding: 10px 14px; background: #fef2f2; border-bottom: 0.5px solid #fca5a5;">
                            ...
                            <p style="font-size: 10px; color: #dc2626; margin: 3px 0 0;">入力後にタブのチェックボックスをオンにしてください</p>
                        </div>
                        <div style="padding: 14px; display: flex; flex-direction: column; gap: 14px;">
                            <div>
                                <label style="font-size: 11px; color: #6b7280; display: block; margin-bottom: 4px;">全体的な指摘内容</label>
                                <textarea wire:model.lazy="generalComments.{{ $currentCarId }}" style="width: 100%; box-sizing: border-box; border: 0.5px solid #d1d5db; border-radius: 8px; padding: 8px 10px; font-size: 12px; color: #111827; background: #f9fafb; resize: vertical; min-height: 70px;" placeholder="例：全体的に写真が暗い状態です"></textarea>
                            </div>
                ...
                <div style="position: sticky; top: 20px; display: flex; flex-direction: column; gap: 10px;">
                    <div style="background: white; border-radius: 12px; border: 0.5px solid #e5e7eb; overflow: hidden;">
                        ...
                        <div style="padding: 14px; display: flex; flex-direction: column; gap: 12px;">
                            ...
                            @if(!empty($currentGeneralComment))
                            <div style="border: 0.5px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
                                ...
                                <div style="padding: 10px 12px;">
                                    <p style="font-size: 11px; color: #6b7280; margin: 0 0 6px; background: #fef2f2; padding: 6px 8px; border-radius: 6px; border-left: 3px solid #E24B4A;">{{ $currentGeneralComment }}</p>
                                    @php $generalResponse = $car->rejection_reason['general_response'] ?? ''; @endphp
                            ...
                            @if(empty($currentGeneralComment) && empty($currentFlaggedImages) && empty($currentRejectionItems))
