...
class RegisterUseCase
{
    ...
    public function execute(RegisterRequest $request): void
    {
        ...
        if (!hash_equals($record->getToken(), hash(PasswordPolicy::HASH_ALGORITHM, $request->token))) {
            throw new \RuntimeException('無効なトークンです');
        }

        if (now()->gt(Carbon::parse($record->getCreatedAt())->addMinutes(PasswordPolicy::RESET_TOKEN_EXPIRE_MINUTES))) {
