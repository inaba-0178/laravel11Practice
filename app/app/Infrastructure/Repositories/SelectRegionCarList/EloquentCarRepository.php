...
class EloquentCarRepository extends BaseRepository implements CarRepositoryInterface
{
    ...
    {
        $query = $this->buildQuery($regionId, $searchParams);
    ...
    {
        return $this->buildQuery($regionId, $searchParams)->count();
    }

    private function buildQuery(int $regionId, array $searchParams): Builder
    {
        $query = $this->model
            ->select([
                'stk_cars.*',
            ...
            ->where('stk_cars.region_id', $regionId)
    ...
    private function applySearchFilters(Builder $query, array $searchParams): void
    {
        ...
        if (!empty($searchParams['carTypes'])) {
            ...
            $query->whereHas('vehicle', function ($q) use ($carTypes) {
                if (in_array('domestic', $carTypes)) $q->where('country_code', 'JP');
                if (in_array('import', $carTypes)) $q->where('country_code', '!=', 'JP');
            });
